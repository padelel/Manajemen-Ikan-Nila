...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Update Data Kolam</h2>
        </template>
                        ...
                        <form @submit.prevent="submit" class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <input v-model="form.nama_kolam" type="text" class="mt-1 w-full rounded-md border-gray-300 shadow-sm" required>
                                </div>
                                <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <input v-model="form.panjang_m" type="number" step="0.1" class="mt-1 w-full rounded-md border-gray-300 shadow-sm" required>
                                </div>
                                
                                <div v-show="form.bentuk_kolam === 'Persegi'">
                                    <label class="block text-sm font-medium text-gray-700">Lebar (meter)</label>
                                    <input v-model="form.lebar_m" type="number" step="0.1" class="mt-1 w-full rounded-md border-gray-300 shadow-sm">
                                </div>

                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Tinggi/Kedalaman (meter)</label>
                                    <input v-model="form.kedalaman_m" type="number" step="0.1" class="mt-1 w-full rounded-md border-gray-300 shadow-sm" required>
                                </div>
                            </div>
