        ...
        <div class="py-8">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <!-- Kotak Kolam Aktif -->
                    <div class="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:-translate-y-1 transition-all duration-300 cursor-default">
                        ...
                        <p class="text-3xl font-black text-slate-900 dark:text-slate-100 transition-colors">{{ ringkasan.totalKolam }} <span class="text-sm font-semibold text-slate-400 dark:text-slate-500">Unit</span></p>
                    </div>
                    
                    <!-- Kotak Total Populasi -->
                    <div class="group relative z-20 hover:z-50 bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:-translate-y-1 transition-all duration-300 cursor-default">
                        ...
                        <p class="text-3xl font-black text-slate-900 dark:text-slate-100 relative z-10 transition-colors">{{ ringkasan.totalIkan.toLocaleString('id-ID') }} <span class="text-sm font-semibold text-slate-400 dark:text-slate-500">Ekor</span></p>
                        
                        <!-- Tooltip Hover Populasi -->
                        <div class="absolute left-0 top-full mt-3 w-full min-w-[220px] bg-slate-900 dark:bg-slate-700 p-5 rounded-2xl shadow-[0_20px_40px_rgb(0,0,0,0.2)] dark:shadow-none opacity-0 invisible group-hover:opacity-100 group-hover:visible translate-y-3 group-hover:translate-y-0 transition-all duration-300 z-50 border border-slate-800 dark:border-slate-600">
                            <div class="absolute -top-2 left-8 w-4 h-4 bg-slate-900 dark:bg-slate-700 rotate-45 border-t border-l border-slate-800 dark:border-slate-600"></div>
                    ...
                    <div class="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:-translate-y-1 transition-all duration-300 cursor-default">
                        ...
                        <p class="text-3xl font-black text-slate-900 dark:text-slate-100 transition-colors">{{ ringkasan.totalBiomassaKg.toLocaleString('id-ID') }} <span class="text-sm font-semibold text-slate-400 dark:text-slate-500">Kg</span></p>
                    </div>
                    ...
                    <div class="group relative z-20 hover:z-50 bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:-translate-y-1 transition-all duration-300 cursor-default">
                        ...
                        <p class="text-3xl font-black text-slate-900 dark:text-slate-100 relative z-10 transition-colors">{{ inventory.stokPakan }} <span class="text-sm font-semibold text-slate-400 dark:text-slate-500">Kg</span></p>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <!-- Grafik Pakan -->
                    <div class="lg:col-span-2 bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col relative z-0 transition-colors duration-300">
                    ...
                    <div class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_10px_40px_rgb(0,0,0,0.2)] flex flex-col relative overflow-hidden z-0 transition-colors duration-300">
                        <!-- Efek Cahaya / Glow -->
                        ...
                        <div class="relative z-10 flex-1 flex flex-col">
                            ...
                            <div class="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar max-h-[350px]">
                                <!-- List Item Pakan -->
                                <div v-for="item in inventory_list" :key="item.id" class="p-5 rounded-2xl border border-slate-100 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-700/30 transition-all hover:bg-slate-100 dark:hover:bg-slate-700/50">
                                    <div class="flex justify-between items-start mb-3">
                                        <div class="max-w-[65%]">
                                            <p class="text-slate-800 dark:text-slate-100 font-bold text-base truncate transition-colors duration-300">{{ item.nama }}</p>
                                        ...
                                        <!-- Label Status Dinamis Light & Dark Mode -->
                                        <span :class="{
                                        ...
                                        }" class="px-2 py-1 rounded-md text-[10px] font-bold uppercase border transition-colors duration-300">
                                            {{ item.status }}
                                        </span>
                                    </div>
                                    ...
                                    <div class="flex items-baseline gap-2 mt-2">
                                        <template v-if="item.rata_rata > 0">
                                            <span class="text-4xl font-black text-slate-900 dark:text-white transition-colors duration-300">{{ item.estimasi }}</span>
                                            <span class="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider transition-colors duration-300">Hari lagi</span>
                                        </template>
                                        <p v-else class="text-xs text-slate-500 dark:text-slate-400 italic mt-2 transition-colors duration-300">Belum cukup data historis.</p>
                                    </div>
                                </div>
                            ...
                            <!-- Tombol Aksi -->
                            <div class="mt-8 pt-6 border-t border-slate-100 dark:border-slate-700 flex gap-3 transition-colors duration-300">
