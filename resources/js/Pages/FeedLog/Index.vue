...
// Fungsi Evaluasi Status Dosis Pakan dengan dukungan Dark Mode
const getStatusDosis = (aktual, rekomendasi) => {
    ...
    if (Math.abs(selisih) <= margin) {
        return { label: 'Sesuai Target', class: 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20' };
    } else if (selisih > margin) {
        return { label: 'Berlebih (Over)', class: 'bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-500/20' };
    } else {
        return { label: 'Kurang (Under)', class: 'bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 border-rose-200 dark:border-rose-500/20' };
    }
};
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-end">
                <div>
                    <h2 class="text-2xl font-bold text-slate-900 dark:text-slate-100 tracking-tight transition-colors duration-300">Riwayat Pemberian Pakan</h2>
                    <p class="text-sm text-slate-500 dark:text-slate-400 mt-1 transition-colors duration-300">Pantau dosis aktual, komposisi pakan, dan tingkat kepatuhan terhadap sistem.</p>
                </div>
            </div>
        </template>
        ...
        <div class="py-8">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-800 overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] sm:rounded-3xl border border-slate-100 dark:border-slate-700 transition-colors duration-300">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            <thead class="bg-slate-50/50 dark:bg-slate-700/50 border-b border-slate-100 dark:border-slate-700 transition-colors duration-300">
                                <tr>
                                    <th class="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-widest w-36">Tanggal</th>
                                    <th class="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-widest">Kolam</th>
                                    <th class="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-widest text-center">Komposisi Pakan</th>
                                    <th class="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-widest">Evaluasi Dosis</th>
                                    <th class="px-6 py-5 text-[10px] font-bold text-slate-400 dark:text-slate-300 uppercase tracking-widest">Petugas</th>
                                </tr>
                            </thead>
                            
                            <tbody class="text-sm">
                                <tr v-for="log in feedLogs.data" :key="log.id" class="border-b border-slate-50 dark:border-slate-700/50 hover:bg-slate-50/80 dark:hover:bg-slate-700/50 transition duration-200">
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="flex items-center gap-3">
                                            <div class="h-9 w-9 rounded-xl bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-blue-500 dark:text-blue-400 border border-blue-100/50 dark:border-blue-500/20 shrink-0 transition-colors">
                                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                                                </svg>
                                            </div>
                                            <div>
                                                <p class="font-bold text-slate-900 dark:text-slate-100 text-base leading-tight transition-colors">{{ log.kolam.nama_kolam }}</p>
                                                <p class="text-[10px] font-bold text-indigo-500 dark:text-indigo-400 uppercase tracking-widest mt-0.5 transition-colors">{{ log.frekuensi }}x Pemberian</p>
                                            </div>
                                        </div>
                                    </td>
                                    
                                    <td class="px-6 py-5">
                                        <div class="bg-slate-50/80 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-600 rounded-xl p-3 inline-block min-w-[200px] transition-colors">
                                            <div class="flex flex-col gap-1.5">
                                                <div v-for="detail in log.details" :key="detail.id" class="flex justify-between items-center gap-4 text-xs">
                                                    <div class="flex items-center gap-2 truncate">
                                                        <span class="w-1.5 h-1.5 rounded-full bg-slate-300 dark:bg-slate-500 shrink-0 transition-colors"></span>
                                                        <span class="font-semibold text-slate-600 dark:text-slate-300 truncate transition-colors">{{ detail.inventory?.nama_pakan }}</span>
                                                    </div>
                                                    <span class="font-black text-slate-800 dark:text-slate-200 whitespace-nowrap transition-colors">{{ detail.jumlah_kg }} <span class="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase transition-colors">Kg</span></span>
                                                </div>
                                                <div v-if="!log.details || log.details.length === 0" class="text-xs text-slate-400 dark:text-slate-500 italic transition-colors">
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="flex flex-col gap-2.5">
                                            <span :class="getStatusDosis(log.pakan_aktual, log.rekomendasi_sistem).class" class="inline-flex items-center justify-center px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-widest w-max border shadow-sm transition-colors duration-300">
                                                {{ getStatusDosis(log.pakan_aktual, log.rekomendasi_sistem).label }}
                                            </span>
                                            <div class="flex items-center gap-3 text-xs">
                                                <div class="flex flex-col">
                                                    <span class="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase transition-colors">Aktual</span>
                                                    <span class="font-black text-slate-900 dark:text-slate-100 transition-colors">{{ log.pakan_aktual }} <span class="text-[10px] text-slate-500 dark:text-slate-400">Kg</span></span>
                                                </div>
                                                <span class="text-slate-300 dark:text-slate-600 transition-colors">/</span>
                                                <div class="flex flex-col">
                                                    <span class="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase transition-colors">Target AI</span>
                                                    <span class="font-black text-slate-500 dark:text-slate-400 transition-colors">{{ log.rekomendasi_sistem }} <span class="text-[10px] text-slate-400 dark:text-slate-500">Kg</span></span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                ...
                                <tr v-if="feedLogs.data.length === 0">
                                    <td colspan="5" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-slate-500 dark:text-slate-400 font-medium text-sm mb-2 transition-colors">Pencarian tidak menemukan hasil pakan.</p>
                    ...
                    <div class="bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-700 p-4 flex flex-col md:flex-row items-center justify-between gap-4 transition-colors duration-300" v-if="feedLogs.links && feedLogs.links.length > 3">
                        <div class="text-sm text-slate-500 dark:text-slate-400 font-medium transition-colors">
                            Menampilkan <span class="font-bold text-slate-800 dark:text-slate-200">{{ feedLogs.from }}</span> - <span class="font-bold text-slate-800 dark:text-slate-200">{{ feedLogs.to }}</span> dari total <span class="font-bold text-slate-800 dark:text-slate-200">{{ feedLogs.total }}</span> data.
                        </div>
