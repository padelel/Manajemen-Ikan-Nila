...
const form = useForm({
    nama_pakan: '',
    total_stok_kg: '',
    terakhir_update: new Date().toISOString().split('T')[0] // Set tanggal hari ini default
});
...
<template>
    <Head title="Tambah Pakan" />
    <AuthenticatedLayout>
        <template #header><h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Stok Pakan</h2></template>
        <div class="py-12"><div class="max-w-2xl mx-auto sm:px-6 lg:px-8"><div class="bg-white p-8 shadow-sm sm:rounded-lg">
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="flex justify-end gap-4 mt-6">
                    ...
                    <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-md hover:bg-blue-700" :disabled="form.processing">Simpan</button>
                </div>
            </form>
        </div></div></div>
    </AuthenticatedLayout>
