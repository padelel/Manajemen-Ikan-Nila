...
const props = defineProps({
    chartPopulasi: { type: Object, required: true },
    events: { type: Object, default: () => ({}) },
});

// ─── Warna per tipe event ───────────────────────────────────────────────────
const EVENT_COLORS = {
    panen_full: "#10b981", // emerald
    panen_parsial: "#f59e0b", // amber
    kematian: "#ef4444", // red
    tebar: "#3b82f6", // blue
};
...
const eventLinePlugin = {
    ...
    afterDatasetsDraw(chart, _args, pluginOpts) {
        ...
        Object.entries(evts).forEach(([lbl, list]) => {
            ...
            ctx.beginPath();
            ctx.setLineDash([5, 4]);
            ctx.strokeStyle = hex + "b3"; // ~70% opacity
            ctx.lineWidth = 1.5;
            ...
            // Badge label di area padding atas (di atas chartArea)
            const badgeText =
                type === "panen_full"
                    ? "PANEN FULL"
                    : type === "panen_parsial"
            ...
            const bw = tw + 10;
            const bh = 15;
            // Jika label mendekati tepi kanan, geser ke kiri
            const bx =
...
const computedChartData = computed(() => {
    if (!props.chartPopulasi?.datasets?.length) return props.chartPopulasi;

    const labels = props.chartPopulasi.labels ?? [];
    const evts = props.events ?? {};

    const pointBg = labels.map((lbl) => {
        const t = primaryType(evts[lbl]);
        return t ? EVENT_COLORS[t] : "#f59e0b";
    });

    const pointR = labels.map((lbl) => ((evts[lbl] ?? []).length > 0 ? 8 : 3));
    ...
    return {
        ...props.chartPopulasi,
        datasets: props.chartPopulasi.datasets.map((ds) => ({
            ...ds,
...
// ─── Opsi chart (computed agar tooltip reaktif dengan events) ────────────────
const chartOptions = computed(() => {
    ...
    return {
        ...
        layout: { padding: { top: 28 } }, // Ruang untuk badge label
        plugins: {
            ...
            tooltip: {
                ...
                callbacks: {
                    ...
                    label: (ctx) => {
                        ...
                        const lbl = ctx.label;
                        const list = evts[lbl] ?? [];
                        const isFull = list.some(
                            (e) => e.type === "panen_full",
                        );
                        if (val === 0 && isFull) {
                            return "  Populasi: 0 ekor (kolam dikosongkan)";
                        }
                        return `  Populasi: ${val.toLocaleString("id-ID")} ekor`;
                    },
                    afterBody: (items) => {
            ...
            // Teruskan events ke plugin lewat opsi plugin
            populationEventLine: { events: evts },
        },
        scales: {
            y: {
                beginAtZero: true,
                ...
                ticks: {
                    color: "#94a3b8",
                    font: { size: 11 },
                    callback: (v) => v.toLocaleString("id-ID") + " Ekor",
                },
            },
            x: {
                grid: { display: false },
...
// ─── Daftar event terurut untuk timeline di bawah grafik ─────────────────────
...
const eventTimelineIcon = {
    panen_full: "M5 13l4 4L19 7", // check
    panen_parsial: "M5 13l4 4L19 7",
    kematian: "M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z", // info/warning
    tebar: "M12 4v16m8-8H4", // plus
};
</script>
    ...
    >
        ...
        >
            <div>
                ...
                >
                    Pergerakan Populasi Ikan
                </h3>
                <span
                    class="inline-block mt-1 px-3 py-1 bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 text-[10px] font-bold rounded-lg uppercase tracking-wider border border-amber-100 dark:border-amber-500/20 transition-colors"
                    >Tren Penebaran & Kematian</span
                >
            </div>
            ...
            <div class="flex flex-wrap gap-x-4 gap-y-2 justify-end max-w-lg">
                <!-- Legend dataset -->
                <div
                    v-for="(dataset, idx) in chartPopulasi?.datasets"
                    :key="idx"
