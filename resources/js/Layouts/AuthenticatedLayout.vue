...
<template>
    <div class="flex h-screen bg-gray-100 font-sans overflow-hidden">
        ...
        <aside :class="[
            ...
            'fixed inset-y-0 left-0 z-50 flex flex-col bg-slate-900 text-slate-300 transition-all duration-300 ease-in-out md:relative shadow-xl'
        ]">
            <!-- Header Sidebar (Logo) -->
            <div class="flex items-center justify-center h-16 bg-slate-950 border-b border-slate-800">
                <Link :href="route('dashboard')" class="flex items-center justify-center w-full">
                    <span v-if="isSidebarOpen" class="text-xl font-black text-white tracking-wider truncate">
                        SMI <span class="text-indigo-500">NILA</span>
                    </span>
                    <span v-else class="text-xl font-black text-indigo-500">
                        NP
                    </span>
                </Link>
            </div>
            ...
            <nav class="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
                ...
                <Link :href="route('dashboard')" @click="closeOnMobile"
                      :class="['flex items-center px-3 py-3 rounded-md text-sm font-medium transition-colors', 
                               route().current('dashboard') ? 'bg-indigo-600 text-white' : 'hover:bg-slate-800 hover:text-white']">
                ...
                <template v-if="$page.props.auth.user.role === 'supervisor'">
                    ...
                    <Link :href="route('kolam.index')" @click="closeOnMobile"
                          :class="['flex items-center px-3 py-3 rounded-md text-sm font-medium transition-colors', 
                                   route().current('kolam.*') ? 'bg-indigo-600 text-white' : 'hover:bg-slate-800 hover:text-white']">
                        <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <Link :href="route('users.index')" @click="closeOnMobile"
                          :class="['flex items-center px-3 py-3 rounded-md text-sm font-medium transition-colors', 
                                   route().current('users.*') ? 'bg-indigo-600 text-white' : 'hover:bg-slate-800 hover:text-white']">
                        <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <Link :href="route('analisis.index')" @click="closeOnMobile"
                          :class="['flex items-center px-3 py-3 rounded-md text-sm font-medium transition-colors', 
                                   route().current('analisis.*') ? 'bg-indigo-600 text-white' : 'hover:bg-slate-800 hover:text-white']">
                        <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
                        </svg>
                        <span class="ml-3 truncate" v-show="isSidebarOpen">Laporan AI</span>
                    </Link>
                </template>
                ...
                <template v-if="$page.props.auth.user.role === 'operator'">
                    ...
                    <Link :href="route('parameter.create')" @click="closeOnMobile"
                          :class="['flex items-center px-3 py-3 rounded-md text-sm font-medium transition-colors', 
                                   route().current('parameter.*') ? 'bg-indigo-600 text-white' : 'hover:bg-slate-800 hover:text-white']">
                        <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <Link :href="route('tebar.index')" @click="closeOnMobile"
                          :class="['flex items-center px-3 py-3 rounded-md text-sm font-medium transition-colors', 
                                   route().current('tebar.*') ? 'bg-indigo-600 text-white' : 'hover:bg-slate-800 hover:text-white']">
                        <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <Link :href="route('kematian.index')" @click="closeOnMobile"
                          :class="['flex items-center px-3 py-3 rounded-md text-sm font-medium transition-colors', 
                                   route().current('kematian.*') ? 'bg-indigo-600 text-white' : 'hover:bg-slate-800 hover:text-white']">
                        <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <Link :href="route('panen.index')" @click="closeOnMobile"
                          :class="['flex items-center px-3 py-3 rounded-md text-sm font-medium transition-colors', 
                                   route().current('panen.*') ? 'bg-indigo-600 text-white' : 'hover:bg-slate-800 hover:text-white']">
                        <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <Link :href="route('tiket.index')" @click="closeOnMobile"
                      :class="['flex items-center px-3 py-3 rounded-md text-sm font-medium transition-colors', 
                               route().current('tiket.*') ? 'bg-indigo-600 text-white' : 'hover:bg-slate-800 hover:text-white']">
