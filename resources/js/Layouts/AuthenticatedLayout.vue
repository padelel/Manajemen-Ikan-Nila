...
<template>
    <div class="min-h-screen bg-slate-50/50 dark:bg-slate-900 transition-colors duration-300 flex">
                ...
                <div class="space-y-1">
                    ...
                    >
                        <p class="text-[10px] font-bold uppercase tracking-widest whitespace-nowrap flex items-center gap-2 transition-colors duration-300"
                           :class="[isMenuUtamaActive ? 'text-blue-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500']">
                    ...
                    <div class="space-y-1">
                        <Link :href="route('dashboard')" 
                            :class="[route().current('dashboard') ? 'bg-slate-900 dark:bg-indigo-600 text-white shadow-lg shadow-slate-200 dark:shadow-none' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50']"
                        ...
                        <Link :href="route('kolam.index')" 
                            :class="[route().current('kolam.*') ? 'bg-slate-900 dark:bg-indigo-600 text-white shadow-lg shadow-slate-200 dark:shadow-none' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50']"
                            class="flex items-center gap-4 px-3 py-2.5 rounded-xl transition-all group overflow-hidden">
                        ...
                        <Link v-if="$page.props.auth.user.role === 'operator'" :href="route('inventory.index')" 
                            :class="[route().current('inventory.*') ? 'bg-slate-900 dark:bg-indigo-600 text-white shadow-lg shadow-slate-200 dark:shadow-none' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50']"
                            class="flex items-center gap-4 px-3 py-2.5 rounded-xl transition-all group overflow-hidden">
                        ...
                        <Link v-if="$page.props.auth.user.role === 'admin'" :href="route('users.index')" 
                            :class="[route().current('users.*') ? 'bg-slate-900 dark:bg-indigo-600 text-white shadow-lg shadow-slate-200 dark:shadow-none' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50']"
                            class="flex items-center gap-4 px-3 py-2.5 rounded-xl transition-all group overflow-hidden">
                            <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
                            </svg>
                            <span :class="[isExpanded ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4']" class="font-medium transition-all duration-300 whitespace-nowrap">Kelola Akun</span>
                        </Link>

                        <Link v-if="$page.props.auth.user.role === 'admin'" :href="route('analisis.index')" 
                            :class="[route().current('analisis.*') ? 'bg-slate-900 dark:bg-indigo-600 text-white shadow-lg shadow-slate-200 dark:shadow-none' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50']"
                            class="flex items-center gap-4 px-3 py-2.5 rounded-xl transition-all group overflow-hidden">
                            <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                            </svg>
                            <span :class="[isExpanded ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4']" class="font-medium transition-all duration-300 whitespace-nowrap">Analisis Kolam</span>
                        </Link>
                        ...
                        >
                            <p class="text-[10px] font-bold uppercase tracking-widest whitespace-nowrap flex items-center gap-2 transition-colors duration-300"
                            :class="[isRiwayatActive ? 'text-blue-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500']">
                        ...
                        <Link :href="route('parameter.index')" 
                            :class="[route().current('parameter.*') ? 'bg-slate-900 dark:bg-indigo-600 text-white shadow-lg shadow-slate-200 dark:shadow-none' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50']"
                            class="flex items-center gap-4 px-3 py-2.5 rounded-xl transition-all group overflow-hidden">
                        ...
                        <Link :href="route('feedlog.index')" 
                            :class="[route().current('feedlog.*') ? 'bg-slate-900 dark:bg-indigo-600 text-white shadow-lg shadow-slate-200 dark:shadow-none' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50']"
                            class="flex items-center gap-4 px-3 py-2.5 rounded-xl transition-all group overflow-hidden">
                        ...
                        <Link v-if="$page.props.auth.user.role === 'admin'" :href="route('inventory.history')" 
                            :class="[route().current('inventory.history') ? 'bg-slate-900 dark:bg-indigo-600 text-white shadow-lg shadow-slate-200 dark:shadow-none' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50']"
                            class="flex items-center gap-4 px-3 py-2.5 rounded-xl transition-all group overflow-hidden">
                            <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        >
                            <p class="text-[10px] font-bold uppercase tracking-widest whitespace-nowrap flex items-center gap-2 transition-colors duration-300"
                               :class="[isPopulasiActive ? 'text-blue-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500']">
                        ...
                        <Link :href="route('kematian.index')" 
                            :class="[route().current('kematian.*') ? 'bg-slate-900 dark:bg-indigo-600 text-white shadow-lg shadow-slate-200 dark:shadow-none' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50']"
                            class="flex items-center gap-4 px-3 py-2.5 rounded-xl transition-all group overflow-hidden">
                        ...
                        <Link :href="route('panen.index')" 
                            :class="[route().current('panen.*') ? 'bg-slate-900 dark:bg-indigo-600 text-white shadow-lg shadow-slate-200 dark:shadow-none' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50']"
                            class="flex items-center gap-4 px-3 py-2.5 rounded-xl transition-all group overflow-hidden">
                            <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <Link :href="route('transfer.index')" 
                            :class="[route().current('transfer.*') ? 'bg-slate-900 dark:bg-indigo-600 text-white shadow-lg shadow-slate-200 dark:shadow-none' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50']"
                            class="flex items-center gap-4 px-3 py-2.5 rounded-xl transition-all group overflow-hidden">
                            <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <Link :href="route('tebar.index')" 
                            :class="[route().current('tebar.*') ? 'bg-slate-900 dark:bg-indigo-600 text-white shadow-lg shadow-slate-200 dark:shadow-none' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50']"
                            class="flex items-center gap-4 px-3 py-2.5 rounded-xl transition-all group overflow-hidden">
                            <svg class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div :class="[isExpanded ? 'md:ml-64' : 'md:ml-20']" class="flex-1 transition-all duration-300 ease-in-out flex flex-col">
            <header class="sticky top-0 z-40 h-16 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border-b border-slate-200/60 dark:border-slate-700/60 px-6 flex items-center justify-between transition-colors duration-300">
                <div class="flex items-center gap-4">
                    <button @click="showingNavigationDropdown = !showingNavigationDropdown" class="md:hidden p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg">
                         <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M4 6h16M4 12h16M4 18h16" /></svg>
                    </button>
                    
                    <div class="hidden md:flex items-center gap-2 text-xs font-medium text-slate-400 dark:text-slate-500">
                        <span>Aplikasi</span>
                        <span>/</span>
                        ...
                        <template v-if="subPageTitle">
                            <span>/</span>
                            <span class="text-slate-600 dark:text-slate-400">{{ subPageTitle }}</span>
                        </template>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4 sm:gap-6">
                    <!-- Tombol Input Harian -->
                    <Link v-if="$page.props.auth.user.role === 'operator'" :href="route('operasi.create')" class="hidden sm:block text-xs font-bold uppercase tracking-wider bg-slate-900 dark:bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-slate-800 dark:hover:bg-indigo-500 transition shadow-md shadow-slate-200 dark:shadow-none">
                        + Input Harian
                    </Link>
                    ...
                    <button 
                        ...
                        class="p-2 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-600 focus:outline-none transition-colors duration-200"
            ...
            <main class="p-6 md:p-10 flex-1 text-slate-900 dark:text-slate-100 transition-colors duration-300">
